...
| 終了コード | Claude の動作 |
|---|---|
| `exit 0` | 続行。**stdout がコンテキストに注入されるのは主に `UserPromptSubmit` と `SessionStart`**。他のイベントではトランスクリプト表示に留まる。Claude の判断制御は stdout の JSON（`hookSpecificOutput` の `permissionDecision` など）で行うのが正式な作法 |
| `exit 1` | 非ブロックエラーとして通知（続行） |
| `exit 2` | **ブロック**。ただし何をブロックするかはイベント依存（`PreToolUse`=ツール実行を拒否、`Stop`=停止を阻止、`UserPromptSubmit`=プロンプトを却下）。stderr が Claude に渡る |
